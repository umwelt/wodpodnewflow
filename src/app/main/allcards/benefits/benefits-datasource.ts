...
export class BenefitsDataSource extends DataSource<BenefitsItem> {
  ...
  constructor(private paginator: MatPaginator, private sort: MatSort, private af: AngularFirestore) {
    super();
    var xss = this.af.collection('/benefits_bank').snapshotChanges().pipe(
      map(actions => actions.map(a => {
        const data = a.payload.doc.data() as BenefitsItem;
   ...
   */
  private getPagedData(data: BenefitsItem[]) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: BenefitsItem[]) {
